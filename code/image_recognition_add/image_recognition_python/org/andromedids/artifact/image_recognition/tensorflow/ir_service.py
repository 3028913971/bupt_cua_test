...
def init_recognizer(model_dir=None, top_predictions_number=5):
...
def publish():
    ...
    web_service_publisher.publish_with_soap11(
        [IRService], "org.andromedids/IRService/tensorflow", "IRService-tensorflow", CONFIG["host"], CONFIG["port"])
...
def main():
    ...
    init_recognizer(model_dir=model_dir)
    publish()
    input()
    stop()
